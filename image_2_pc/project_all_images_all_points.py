...
def process_pair(pair_dir, calib_file, calib_resolution, robot_model, tx, ty, tz, angle_x, angle_y, angle_z):
    ...
    # Create manual adjustment matrix.
    ...
    # Prepare an array for colors. Default remains unused since all points inside image will get image colors.
    default_color = np.array([0.5, 0.5, 0.5])
    colors = np.tile(default_color, (adjusted_points.shape[0], 1))

    # For each point, if its projection is valid and depth positive, assign the color from the image.
    ...
    # Create the final colored point cloud.
...
def main():
    ...
    calib_resolution = (1544, 2064)  # (height, width)
    # Robot model: "Spot"
    robot_model = "Spot"
