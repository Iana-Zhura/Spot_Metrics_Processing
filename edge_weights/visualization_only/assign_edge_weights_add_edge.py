...
def compute_edge_values_with_new_edge(graph, metrics_file, new_from_id, new_to_id,
                                      rotation_z=0, rotation_y=0, translation=(0,0,0),
                                      metric_key="average_effort"):
    """
    Compute raw and normalized edge weights between anchors and add a new edge between
    the selected waypoints/anchors (new_from_id and new_to_id). The function:
      1. Loads anchor IDs, anchor positions, and metrics.
      2. Transforms anchor positions.
      3. Computes the raw weight for each edge (raw_weight = average metric of endpoints +
         Euclidean distance between them).
      4. Adds the new edge (if its endpoints exist) and appends its raw weight.
      5. Normalizes all raw weights to [0,1].
      6. Computes the mean time spent at the two endpoints for each edge.
    
    Args:
        graph (object): The graph loaded by your SDK.
        metrics_file (str): Path to the JSON file with metrics.
        new_from_id (str): The starting anchor ID for the new edge.
        new_to_id (str): The ending anchor ID for the new edge.
        rotation_z (float, optional): Rotation angle around Z. Default is 0.
        rotation_y (float, optional): Rotation angle around Y. Default is 0.
        translation (tuple/list, optional): Translation vector. Default is (0,0,0).
        metric_key (str, optional): Key to select which metric to use. Default is "average_effort".
    
    Returns:
        export_data (dict): Mapping of "from -> to" to a dict with:
              - "from": from_id
              - "to": to_id
              - "raw_weight": computed raw weight
              - "normalized_weight": normalized weight (0 to 1)
              - "mean_time_spent": average time spent at the endpoints
        transformed_anchor_map (dict): Transformed anchor positions.
        waypoint_to_numeric (dict): Mapping of each waypoint_id to its original numerical key.
    """
    # 1. Load anchors and metrics.
    anchor_ids = extract_anchor_ids(graph)
    anchor_map = extract_anchor_map(graph)
    transformed_anchor_map = transform_anchor_map(anchor_map, rotation_z, rotation_y, translation)
    metrics = load_metrics(metrics_file, anchor_ids, metric_key=metric_key)
    
    # 2. Load raw metrics to get time_spent and original numeric keys.
    with open(metrics_file, 'r') as f:
        raw_json = json.load(f)
    waypoint_to_numeric = {}
    time_spent_map = {}
    ...
    # 3. Extract existing edges and compute raw weights.
    edges = extract_edges_from_graph(graph)
    raw_edges = []
    for from_id, to_id, _ in edges:
        if from_id in transformed_anchor_map and to_id in transformed_anchor_map:
    ...
    # 4. Add the new edge.
    if new_from_id in transformed_anchor_map and new_to_id in transformed_anchor_map:
        pos1 = transformed_anchor_map[new_from_id]
        pos2 = transformed_anchor_map[new_to_id]
        new_raw_weight = calc_edge_value(new_from_id, new_to_id, metrics, pos1, pos2)
        raw_edges.append((new_from_id, new_to_id, new_raw_weight))
    else:
        print("New edge anchor IDs not found in transformed anchor map.")
    
    # 5. Normalize the raw edge weights.
    normalized_edges = normalize_edge_weights(raw_edges)
    
    # 6. Build the export data using normalized weights and compute mean time spent.
    export_data = {}
    for (from_id, to_id, raw_weight), (_, _, norm_weight) in zip(raw_edges, normalized_edges):
        time1 = time_spent_map.get(from_id, 0)
        time2 = time_spent_map.get(to_id, 0)
...
if __name__ == "__main__":
    ...
    if add_new_edge:
        ...
        # Compute edge values and include the new edge.
        edge_data, transformed_anchor_map, waypoint_to_numeric = compute_edge_values_with_new_edge(
            graph, metrics_file, new_from_id, new_to_id,
            rotation_z, rotation_y, translation, metric_key="average_effort"
        )
    ...
    if True:
        # get custom weights for two selected points
        anchor1_id = "gabby-snake-GPHX1+oqSU0HJq.KSqKn5Q=="
        anchor2_id = "skimpy-rodent-OEdIKag2mGSIuDu55.RM7A=="

        anchor_map = extract_anchor_map(graph)
        transformed_anchor_map = transform_anchor_map(anchor_map, rotation_z, rotation_y, translation)

        pos1 = transformed_anchor_map[anchor1_id]
        pos2 = transformed_anchor_map[anchor2_id]

        custom_weight = calc_edge_value(anchor1_id, anchor2_id, metrics, pos1, pos2)
        normalized_cw = normalize_custom_edge(custom_weight)

        print(f"Custom normalized weight between {anchor1_id} and {anchor2_id}: {normalized_cw:.2f}")
